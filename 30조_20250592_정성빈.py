...
def add_msg(role, content):
    """대화 기록을 Session State에 누적 저장"""
    st.session_state.chat_history.append({"role": role, "content": content})

def update_metrics(stress, conf, stab, satis):
    """4대 심리 지표 점수 연산 및 유효 구간(0~100) 보정"""
...
# ================= 3. 메인 프로세스 및 챗봇 라우팅 로직 ================= #

# [Step 0] 사용자 정보 입력(로그인) 인터페이스
...
# [Step 1~10] 대화형 심리 분석 인터페이스
    ...
    def handle_choice(user_text, next_step, cause=None, stress=0, conf=0, stab=0, satis=0, bot_reply=""):
        """사용자 응답에 따른 지표 연산 및 단계(Step) 전환 컨트롤러"""
    ...
    # [Step 2] 심리적 자극 요인 분석 (S/N 분기)
        ...
        if 'S' in st.session_state.user_mbti: 
            ...
            if st.button("1. 눈앞에 주어진 현실적인 과제나 목표를 하나씩 달성해 나가는 중입니다."): step2_choice("현실적 목표 달성", "목표 달성", -15, 15, 10, 15)
            if st.button("2. 예상치 못한 지출이나 팍팍한 재정적, 현실적 문제로 머리가 아픕니다."): step2_choice("재정 및 현실 문제", "현실적 압박", 20, -10, -10, -15)
            if st.button("3. 일상의 소소한 루틴을 안정적으로 지키며 평안함을 느끼고 있습니다."): step2_choice("일상의 안정감", "일상의 평안", -10, 5, 15, 10)
            if st.button("4. 끝이 없는 집안일이나 디테일을 챙겨야 하는 과업에 지쳤습니다."): step2_choice("과도한 디테일 업무", "과업 스트레스", 15, 0, -10, -5)
        else: 
            st.markdown("💬 **[질문 2] 최근 당신의 가치관이나 미래에 대해 가장 깊게 생각하게 만드는 주제는 무엇인가요?**")
            if st.button("1. 가슴을 뛰게 하는 새로운 아이디어나 미래의 목표가 생겼습니다."): step2_choice("새로운 영감과 목표", "미래 기대감", -15, 15, 5, 15)
            if st.button("2. 내 미래가 불투명하게 느껴지고 방향성을 잃은 것 같습니다."): step2_choice("미래에 대한 막막함", "미래 불안감", 25, -15, -15, -10)
            if st.button("3. 내가 매일 하고 있는 일의 진정한 가치와 의미를 찾아가고 있습니다."): step2_choice("가치와 의미 발견", "가치관 확립", -10, 10, 15, 10)
            if st.button("4. 지금 하는 일의 의미를 찾지 못해 무기력하고 정체된 기분입니다."): step2_choice("영감의 고갈", "가치관 혼란", 20, -10, -20, -15)

    # [Step 3] 상황 해석 메커니즘 분석 (T/F 분기)
        ...
        if 'T' in st.session_state.user_mbti: 
            ...
            if st.button("1. 복잡한 문제를 논리적으로 분석해 명확한 해결책을 찾았을 때의 뿌듯함"): step3_choice("문제 해결의 쾌감", -15, 20, 10, 15)
            if st.button("2. 합의된 원칙이 무시되는 비효율적인 상황이나 시스템에 대한 답답함"): step3_choice("비효율에 대한 답답함", 20, 0, -15, -10)
            if st.button("3. 내 능력을 온전히 발휘하여 시스템을 개선하고 성과를 낼 때의 성취감"): step3_choice("능력 발휘와 성취", -10, 15, 10, 15)
            if st.button("4. 논리는 없고 감정적으로만 호소하며 억지를 부리는 사람을 대할 때의 피로감"): step3_choice("비논리적 상황의 피로", 20, 5, -15, -10)
        else: 
            st.markdown("💬 **[질문 3] 사람들과의 관계 속에서 당신의 마음을 가장 크게 움직이는 요소는 무엇인가요?**")
            if st.button("1. 누군가와 깊이 공감하고 진심 어린 위로를 주고받았을 때의 따뜻함"): step3_choice("깊은 공감과 연대", -15, 10, 20, 15)
            if st.button("2. 나의 진심 어린 배려가 오해받거나 가볍게 무시당했다고 느낄 때의 상처"): step3_choice("진심이 무시당함", 25, -15, -20, -10)
            if st.button("3. 주변 사람들이 갈등 없이 화목하고 평화롭게 어울리는 모습을 볼 때의 안정감"): step3_choice("관계의 평화와 조화", -10, 5, 15, 10)
            if st.button("4. 내 의도와 다르게 내가 누군가에게 상처를 주었다는 생각에 빠질 때의 죄책감"): step3_choice("타인에게 상처를 줬다는 자책", 25, -20, -25, -10)

    # [Step 4] 4대 기질별(NF/NT/SJ/SP) 내면적 동기 파악
        ...
        if 'N' in mbti and 'F' in mbti: 
            ...
            if st.button("1. 나만의 고유한 정체성과 잠재력을 발견하고 성장해 나가는 기쁨"): step4_choice("자아 성장", -15, 15, 15, 15)
            if st.button("2. 현실의 벽에 부딪혀 내가 꿈꾸던 이상이 꺾이는 듯한 좌절감"): step4_choice("이상이 꺾인 좌절감", 20, -15, -15, -15)
            if st.button("3. 타인에게 긍정적인 영향을 주고 세상을 더 낫게 만든다는 보람"): step4_choice("선한 영향력", -15, 10, 15, 20)
            if st.button("4. 진짜 내 모습을 숨기고 세상이 원하는 가면을 써야 하는 답답함"): step4_choice("자아 억눌림", 20, -10, -20, -10)
            
        elif 'N' in mbti and 'T' in mbti:
            st.markdown("💬 **[질문 4] 지적 성장을 중시하는 당신, 현재 당신의 내면을 가장 크게 채우고 있는 감정은 무엇인가요?**")
            if st.button("1. 새로운 지식을 습득하고 나의 전문성을 한 단계 끌어올리는 쾌감"): step4_choice("지적 성장", -15, 20, 10, 15)
            if st.button("2. 무능력한 리더나 비합리적인 의사결정에 휘둘려야 하는 분노"): step4_choice("비합리성에 대한 분노", 25, 10, -20, -10)
            if st.button("3. 내 통제하에 모든 계획과 시스템이 완벽하게 굴러갈 때의 만족감"): step4_choice("통제와 달성", -15, 15, 15, 15)
            if st.button("4. 지적으로 아무런 자극이 없는 지루하고 반복적인 환경에 대한 무료함"): step4_choice("지적 자극 결핍", 15, -5, -10, -20)
            
        elif 'S' in mbti and 'J' in mbti:
            st.markdown("💬 **[질문 4] 책임과 안정을 추구하는 당신, 현재 당신의 내면을 가장 크게 채우고 있는 감정은 무엇인가요?**")
            if st.button("1. 내게 주어진 책임과 역할을 완벽하게 해내어 인정받을 때의 뿌듯함"): step4_choice("책임 완수와 인정", -15, 15, 15, 15)
            if st.button("2. 나 혼자서만 이 모든 상황을 책임지고 감당해야 한다는 막막한 압박감"): step4_choice("과도한 책임감", 25, -15, -20, -15)
            if st.button("3. 예측 가능하고 규칙적인 일상 속에서 느껴지는 굳건한 평온함"): step4_choice("일상의 평온", -15, 10, 20, 15)
            if st.button("4. 갑작스러운 변화나 타인의 무책임함으로 인해 일상의 질서가 깨지는 스트레스"): step4_choice("일상의 붕괴", 20, -10, -25, -10)
            
        else: 
            st.markdown("💬 **[질문 4] 경험과 자유를 사랑하는 당신, 현재 당신의 내면을 가장 크게 채우고 있는 감정은 무엇인가요?**")
            if st.button("1. 새로운 경험이나 액티비티에 즉흥적으로 뛰어들어 즐기는 짜릿함"): step4_choice("새로운 경험의 짜릿함", -15, 15, 10, 20)
            if st.button("2. 융통성 없는 꽉 막힌 규율과 틀에 억지로 내 행동을 맞춰야 하는 답답함"): step4_choice("규율에 대한 답답함", 20, 5, -15, -15)
            if st.button("3. 누구에게도 얽매이지 않고 내가 원하는 대로 자유롭게 시간을 쓸 때의 해방감"): step4_choice("자유와 해방감", -15, 15, 15, 15)
            if st.button("4. 새로운 자극 없이 매일 똑같이 반복되는 쳇바퀴 같은 일상의 지루함"): step4_choice("자극 없는 지루함", 15, -5, -10, -20)

    # [Step 5] 스트레스 발현 행동 분석 (E/I 분기)
        ...
        if 'E' in st.session_state.user_mbti:
            ...
            if st.button("1. 에너지가 넘쳐 사람들을 만나고 수다를 떨며 감정을 적극적으로 나눕니다."): step5_choice("적극적인 감정 교류", -15, 10, 10, 15)
            if st.button("2. 충동적으로 돈을 쓰거나 평소보다 목소리가 커지고 예민해집니다."): step5_choice("충동적 소비/예민함", 20, -5, -20, -10)
            if st.button("3. 새로운 모임이나 활동적인 스포츠에 즉흥적으로 뛰어들어 에너지를 발산합니다."): step5_choice("활동적 에너지 발산", -10, 15, 5, 15)
            if st.button("4. 주변 사람들에게 짜증을 내거나 마음속 불만을 여과 없이 쏟아내게 됩니다."): step5_choice("짜증과 불만 표출", 20, -10, -15, -10)
        else:
            st.markdown("💬 **[질문 5] 긍정적이든 부정적이든 감정의 에너지가 크게 요동칠 때, 당신의 행동은 주로 어떻게 변하나요?**")
            if st.button("1. 나만의 안전한 공간으로 들어가 온전한 휴식을 취하며 에너지를 채웁니다."): step5_choice("안전한 고립과 충전", -15, 5, 20, 15)
            if st.button("2. 모든 연락을 차단하고 누구와도 말하고 싶지 않아 깊이 고립되어 버립니다."): step5_choice("연락 두절 및 깊은 고립", 15, -5, -15, -5)
            if st.button("3. 조용히 일기를 쓰거나 생각을 정리하며 감정을 내면에서 차분히 소화시킵니다."): step5_choice("차분한 내면 소화", -15, 10, 15, 10)
            if st.button("4. 생각만 꼬리를 물고 이어지며 무기력하게 침대에 누워 잠만 자고 싶어집니다."): step5_choice("과수면과 생각 과잉", 20, -10, -15, -10)

    # [Step 6] 자아상 및 인지적 오류 진단 (T/F 분기)
        ...
        if 'T' in st.session_state.user_mbti:
            ...
            if st.button("1. '나는 어떤 문제든 이성적으로 분석하고 해결할 능력이 있어'라는 확신"): step6_choice("이성적 확신", -15, 25, 15, 10)
            if st.button("2. '내 능력이 부족해서 이런 일도 완벽하게 해결 못하는구나'라는 자책"): step6_choice("능력에 대한 자책", 15, -20, -10, -10)
            if st.button("3. '감정에 휘둘리지 않고 상황을 객관적으로 통제할 수 있다'는 자신감"): step6_choice("객관적 통제감", -10, 20, 15, 10)
            if st.button("4. '왜 세상 사람들은 이렇게 비합리적일까?'라며 타인을 향하는 냉소"): step6_choice("타인에 대한 냉소", 15, 5, -20, -10)
        else:
            st.markdown("💬 **[질문 6] 당신이 스스로를 바라보는 무의식적인 생각의 습관은 다음 중 어느 쪽에 가깝나요?**")
            if st.button("1. '나의 진심과 배려는 결국 주변 사람들에게 따뜻한 위로가 될 거야'라는 믿음"): step6_choice("진심에 대한 믿음", -15, 15, 20, 15)
            if st.button("2. '나 때문에 다른 사람들이 피해를 보거나 실망하면 어쩌지?'라는 불안"): step6_choice("타인에 대한 죄책감", 20, -15, -15, -10)
            if st.button("3. '나는 상처받더라도 타인과 깊이 교감하며 성장할 수 있는 사람이야'라는 수용"): step6_choice("교감과 수용력", -15, 20, 15, 15)
            if st.button("4. '결국 아무도 내 진짜 마음이나 상처를 온전히 알아주지 않을 거야'라는 소외감"): step6_choice("깊은 고립감과 소외감", 25, -10, -25, -15)

    # [Step 7] 이상적 지지 형태 확인 (공통)
    ...
    # [Step 9] 주도적 감정 해소 방식 분석 (E/I, T/F 혼합 분기)
        ...
        if is_e:
            ...
            if st.button("1. 누구와도 연락하지 않고 온전히 혼자만의 공간에 깊숙이 들어갑니다."): step9_choice("혼자만의 공간", -10, 0, 10, 5)
    ...
    # ==============================================================================
    # [Step 11] 최종 결과 도출 (결과 그래프 시각화 및 맞춤형 처방전 렌더링)
    # ==============================================================================
    elif st.session_state.step == 11: 
        st.success("🎉 10단계 심층 분석이 완료되었습니다!")
        st.subheader("📊 4대 심리 지표 결과")
        
        # 11.1. Matplotlib 기반 결과 그래프 생성
        ...
        # 11.4. 산출물 다운로드 & 세션 초기화 UI 구성 (비율 7:3)
